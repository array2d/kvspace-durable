...
- 坐标段整体是**父目录 `m.` 下的一个成员名**，不是多级路径。
- 坐标个数**恒等于 ndim**，不足或超出即非法。
- 坐标是十进制非负整数，`,` 分隔，**无空格**，`[` `]` 包围。
...
| `strkeymapindex` | ndim ≥ 1 | `m.[s0,...]` | 散 key ndarray，键是整数坐标 |
...
**objindex 成员名**禁止：`/` `.` `[` `]` `\n` `\r` `\0`、`‥`(U+2025)、`…`(U+2026)、ASCII 控制字符（<0x20）、空串。禁 `[` 保证了 §4 的字面可分。

**strkeymapindex 坐标段**必须完全匹配 `\[[0-9]+(,[0-9]+)*\]`，其余一律非法。
