...
pub fn new_int8(v: &[i8]) -> XValue {
    XValue::Int8(Arr { data: v.to_vec(), dims: dims_from_len(v.len()) })
}
pub fn new_int16(v: &[i16]) -> XValue {
    XValue::Int16(Arr { data: v.to_vec(), dims: dims_from_len(v.len()) })
}
pub fn new_int32(v: &[i32]) -> XValue {
    XValue::Int32(Arr { data: v.to_vec(), dims: dims_from_len(v.len()) })
}
pub fn new_int64(v: &[i64]) -> XValue {
    XValue::Int64(Arr { data: v.to_vec(), dims: dims_from_len(v.len()) })
}

pub fn decode_int8(body: &[u8], dims: &[i32]) -> Arr<i8> {
    Arr { data: body.iter().map(|&b| b as i8).collect(), dims: dims.to_vec() }
}
pub fn decode_int16(body: &[u8], dims: &[i32]) -> Arr<i16> {
    Arr { data: body.chunks_exact(2).map(|c| i16::from_le_bytes([c[0], c[1]])).collect(), dims: dims.to_vec() }
}
pub fn decode_int32(body: &[u8], dims: &[i32]) -> Arr<i32> {
    Arr { data: body.chunks_exact(4).map(|c| i32::from_le_bytes([c[0], c[1], c[2], c[3]])).collect(), dims: dims.to_vec() }
}
pub fn decode_int64(body: &[u8], dims: &[i32]) -> Arr<i64> {
    Arr { data: body.chunks_exact(8).map(|c| i64::from_le_bytes(c.try_into().unwrap())).collect(), dims: dims.to_vec() }
}
